...
struct eventfd_ctx *user_notifier[MAX_FPGA_DEVICES][MAX_N_REGIONS][N_CTID_MAX];

/// Every time a user interrupt is issued, the mutex is locked, avoiding race conditions until the interrupt has been handled
struct mutex user_notifier_lock[MAX_FPGA_DEVICES][MAX_N_REGIONS][N_CTID_MAX];
...
int32_t interrupt_value[MAX_FPGA_DEVICES][MAX_N_REGIONS][N_CTID_MAX];

int vfpga_register_eventfd(struct vfpga_dev *device, int ctid, int eventfd) {
    int ret_val = 0;
    BUG_ON(!device);
    
    mutex_init(&user_notifier_lock[device->bd_data->dev_id][device->id][ctid]);

    // Retrieve the kernel context from the eventfd file descriptor
    user_notifier[device->bd_data->dev_id][device->id][ctid] = eventfd_ctx_fdget(eventfd);
    if (IS_ERR_OR_NULL(user_notifier[device->bd_data->dev_id][device->id][ctid])) {
        ret_val = PTR_ERR(user_notifier[device->bd_data->dev_id][device->id][ctid]);
        user_notifier[device->bd_data->dev_id][device->id][ctid] = NULL;
...
void vfpga_unregister_eventfd(struct vfpga_dev *device, int ctid) {
    ...
    if (user_notifier[device->bd_data->dev_id][device->id][ctid]) {
        eventfd_ctx_put(user_notifier[device->bd_data->dev_id][device->id][ctid]);
    }
    user_notifier[device->bd_data->dev_id][device->id][ctid] = NULL;
}
