...
Binary classifier on grayscale images derived from FPGA partial bitstreams.
...
python train.py --img-size 512     # sample 512*512 bytes for the 2D view before training
...
- **Input:** raw `.bin` files mapped to an `img_size x img_size` grayscale view via uniform subsampling for 2D experiments
