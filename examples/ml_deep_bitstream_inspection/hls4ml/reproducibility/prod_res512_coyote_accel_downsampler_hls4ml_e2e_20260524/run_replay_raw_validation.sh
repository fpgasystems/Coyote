...
PKG=${PKG:-/pub/scratch/sdeheredia/Coyote/examples/ml_baseline/hls4ml/reproducibility/prod_res512_coyote_accel_downsampler_hls4ml_e2e_20260524}
...
trap 'status=$?; if [ "$status" -eq 0 ]; then notify "prod_res512_manualA_coyote_accel replay OK on $(hostname); log=$LOG"; else notify "prod_res512_manualA_coyote_accel replay FAILED status=$status on $(hostname); log=$LOG"; fi' EXIT

{
  ...
  python "$PKG/replay_validate.py" --package "$PKG" --project-name "prod_res512_manualA_coyote_accel" --program "$PROGRAM"
