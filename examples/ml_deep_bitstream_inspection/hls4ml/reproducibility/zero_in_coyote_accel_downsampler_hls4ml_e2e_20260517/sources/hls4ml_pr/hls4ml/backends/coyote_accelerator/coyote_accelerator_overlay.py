...
class CoyoteOverlay:
    ...
    def predict_raw(self, X: list, y_shape: tuple, batch_size: int = 1):
        """
        Run inference with raw byte inputs.

        Each sample is copied to the generated host library as bytes. The host
        library prepends the raw-length header expected by the HLS wrapper.
        """
        raw_samples = [np.ascontiguousarray(np.asarray(x, dtype=np.uint8).reshape(-1)) for x in X]
        ...
        self.coyote_lib.init_model_inference_raw.restype = ctypes.POINTER(ctypes.c_void_p)

        np_pointer_u8 = np.ctypeslib.ndpointer(dtype=np.uint8, ndim=1, flags='C')
        self.coyote_lib.set_inference_raw_data.argtypes = [
        ...
        y = np.empty((len(raw_samples), *y_shape))
        max_input_bytes = max((len(x) for x in raw_samples), default=0)
        ...
        for x in raw_samples:
            self.coyote_lib.set_inference_raw_data(model, x, len(x), cnt)
            cnt += 1
            if cnt == batch_size:
                self.coyote_lib.flush(model)
