...
LOG_ROOT="${2:-$ROOT/logs/big_ro_training/$(date +%Y%m%d_%H%M%S)}"
TOPIC="${TOPIC:-coyote-build-sdeheredia}"
CONFIG_DIR="configs/hls4ml_big_ro_training"
RESULTS_DIR="results/big_ro_training"
...
p = Path("results/big_ro_training/suite_status.csv")
...
trap 'status=$?; notify "big-RO unbalanced training FAILED status=$status session=$SESSION log=$LOG_ROOT/supervisor.log"; exit $status' ERR

echo "[big-ro] launched session=$SESSION logs=$LOG_ROOT"
notify "big-RO unbalanced training launched: session=$SESSION logs=$LOG_ROOT"

"$PY" scripts/prepare_big_ro_training.py --reuse-existing-configs

echo "[big-ro] start float training jobs=2"
notify "big-RO unbalanced float training started: jobs=2 session=$SESSION logs=$LOG_ROOT/float"
...
notify "big-RO unbalanced QAT P50 training started: jobs=2 session=$SESSION logs=$LOG_ROOT/qat_p50"
"$PY" scripts/run_experiment_configs_parallel.py \
  --configs "$CONFIG_DIR" \
  --phases qat_p50 \
  --stages train \
...
notify "big-RO unbalanced training complete: $(status_summary) session=$SESSION results=$ROOT/$RESULTS_DIR"
