...
def reexec_local_python_if_needed() -> None:
    ...
    candidates = [
        ml_baseline_root / ".venv_hls4ml" / "bin" / "python",
        ml_baseline_root / ".venv" / "bin" / "python",
        project_root / ".venv_hls4ml" / "bin" / "python",
        project_root / ".venv" / "bin" / "python",
        EXAMPLE_ROOT / ".venv_hls4ml" / "bin" / "python",
        EXAMPLE_ROOT / ".venv" / "bin" / "python",
    ]
    current = Path(sys.executable)
    if any(current == candidate or (current.parent == candidate.parent and current.name.startswith("python")) for candidate in candidates):
        return
