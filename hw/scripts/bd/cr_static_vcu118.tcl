  ...
  if {$cnfg(en_ddr) eq 1} { 
    for {set i 0} {$i < $cnfg(n_ddr_chan)} {incr i} {
      for {set j 0} {$j < 2} {incr j} {
        ...
        set cmd "create_bd_addr_seg -range 0x80000000 -offset 0x00000000 \[get_bd_addr_spaces axi_ddr_in_$nn] \[get_bd_addr_segs ddr4_$i/C0_DDR4_MEMORY_MAP/C0_DDR4_ADDRESS_BLOCK] SEG_ddr4_$i\_C0_DDR4_ADDRESS_BLOCK"
        eval $cmd
      }
    }
  }
