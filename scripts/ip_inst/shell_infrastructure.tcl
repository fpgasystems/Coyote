...
# Control crossbars
# AXI Crossbars are only supported on UltraScale+ architectures, not on Versal
# Versal uses the SmartConnect IP, which must be instantiated in a block diagram
# To achieve the same shell functionality, the Versal BD is added in a separate TCL script (hw/bd/cr_xbar.tcl)
if {$cfg(fpga_arch) eq "ultrascale_plus"} {
