...
function make_sim() {
	...
	# running cmake			!!!ADJUST PATH!!!
    if [[ "$CLEAN_SIM_DIRECTORY" -eq 1 ]] || [[ "$BUILD_DIR_EMPTY" ]]; then
        "$CMAKE" "$ABS_PATH/../../examples/04_user_interrupts/hw" "-DFDEV_NAME=u55c" >> "$LOG_FILE" 2>&1
	...
	# patch simulation project				!!!ADJUST PROJECT NAME AND PATH!!!
	echo "Patching simulation project"
	vivado -mode tcl sim/example_04_user_interrupts.xpr < "$ABS_PATH/sim_patch_04.tcl" >> "$LOG_FILE" 2>&1
	
	if [[ "$NOGUI" -eq 0 ]]; then
		# open vivado						!!!ADJUST PROJECT NAME!!!
		echo "Opening Vivado..."
		vivado "$ABS_PATH/../../build_sim/sim/example_04_user_interrupts.xpr" >> "$LOG_FILE" 2>&1
    else
