...
function make_sim() {
	...
	vivado -mode tcl sim/example_07_perf_fpga.xpr < "$ABS_PATH/sim_patch_07.tcl" >> "$LOG_FILE" 2>&1
	
	if [[ "$NOGUI" -eq 0 ]]; then
		# open vivado						!!!ADJUST PROJECT NAME!!!
		echo "Opening Vivado..."
		vivado "$ABS_PATH/../../build_sim/sim/example_07_perf_fpga.xpr" >> "$LOG_FILE" 2>&1
    else
