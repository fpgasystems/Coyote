...
function make_sim() {
    ...
    if [[ "$CLEAN_SIM_DIRECTORY" -eq 1 ]] || [[ "$BUILD_DIR_EMPTY" ]]; then
        "$CMAKE" "$ABS_PATH/../../examples/08_perf_rdma/hw" "-DFDEV_NAME=u55c" >> "$LOG_FILE" 2>&1
	...
	vivado -mode tcl sim/example_08_perf_rdma.xpr < "$ABS_PATH/sim_patch_08.tcl" >> "$LOG_FILE" 2>&1
	
	if [[ "$NOGUI" -eq 0 ]]; then
		# open vivado						!!!ADJUST PROJECT NAME!!!
		echo "Opening Vivado..."
		vivado "$ABS_PATH/../../build_sim/sim/example_08_perf_rdma.xpr" >> "$LOG_FILE" 2>&1
    else
