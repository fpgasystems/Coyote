...
class fDev {
	...
	/* Data movement */
	void _read(uint64_t *vaddr, uint32_t len, uint8_t dest = 0, bool stream = true, bool clr_stat = true, bool poll = true);
	void _write(uint64_t *vaddr, uint32_t len,  uint8_t dest = 0, bool stream = true, bool clr_stat = true, bool poll = true);
	void _transfer(uint64_t *vaddr_src, uint64_t* vaddr_dst, uint32_t len_src, uint32_t len_dst,  uint8_t dest = 0, bool stream = true, bool clr_stat = true, bool poll = true);
...
public:
	...
	template <typename _Vaddr = uint64_t>
	void read(_Vaddr *vaddr, uint32_t len, uint8_t dest = 0, bool stream = true, bool clr_stat = true, bool poll = false) {
		_read((uint64_t*)vaddr, len, dest, stream, clr_stat, poll);
	}

	template <typename _Vaddr = uint64_t>
	void write(_Vaddr *vaddr, uint32_t len, uint8_t dest = 0, bool stream = true, bool clr_stat = true, bool poll = false) {
		_write((uint64_t*)vaddr, len, dest, stream, clr_stat, poll);
	}

	template <typename _Vaddr = uint64_t>
	void transfer(_Vaddr *vaddr_src, _Vaddr *vaddr_dst, uint32_t len_src, uint32_t len_dst, uint8_t dest_src = 0, uint8_t dest_dst = 0, bool stream = true, bool clr_stat = true, bool poll = false) {
		_transfer((uint64_t*)vaddr_src, (uint64_t*)vaddr_dst, len_src, len_dst, dest_src, dest_dst, stream, clr_stat, poll);
	}
	...
	void writeContext(fQPair *pair);
    // Write connection
	void writeConnection(fQPair *pair, uint32_t port);

    // RDMA ops
    bool postWrite(fQPair *pair, uint64_t l_offs, uint64_t r_offs, uint32_t size);
    bool postRead(fQPair *pair, uint64_t l_offs, uint64_t r_offs, uint32_t size);
	bool postFarview(fQPair *pair, uint64_t l_offs, uint64_t r_offs, uint32_t size, uint64_t params);
#endif
};
