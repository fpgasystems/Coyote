...
void fDev::reconfigure(uint64_t *vaddr, uint64_t len) {
	 ...
	 */
	void fDev::_read(uint64_t* vaddr, uint32_t len, bool stream, bool poll, bool clr_stat) {
		// Check outstanding
		...
		uint64_t ctrl_cmd = CTRL_START_RD | (clr_stat ? CTRL_CLR_STAT_RD : 0x0) | (stream ? CTRL_STREAM_RD : 0x0);
	 ...
	 * @param: clr_stat - prior status clear
	 */
	void fDev::_write(uint64_t* vaddr, uint32_t len, bool stream, bool poll, bool clr_stat) {
		// Check outstanding
		...
		uint64_t ctrl_cmd = CTRL_START_WR | (clr_stat ? CTRL_CLR_STAT_WR : 0x0) | (stream ? CTRL_STREAM_WR : 0x0);
	 ...
	 * @param: clr_stat - prior status clear
	 */
	void fDev::_transfer(uint64_t* vaddr_src, uint64_t* vaddr_dst, uint32_t len_src, uint32_t len_dst, bool stream, bool poll, bool clr_stat) {
		...
		uint64_t ctrl_cmd = CTRL_START_WR | (clr_stat ? CTRL_CLR_STAT_WR : 0x0) | (stream ? CTRL_STREAM_WR : 0x0) |
							CTRL_START_RD | (clr_stat ? CTRL_CLR_STAT_RD : 0x0) | (stream ? CTRL_STREAM_RD : 0x0);
	 ...
	 * @param: clr_stat - prior status clear
	 */
	void fDev::read(uint64_t* vaddr, uint32_t len, bool poll, bool clr_stat) {
		// Check outstanding
		while (rd_cmd_cnt > (cmd_fifo_depth - cmd_fifo_thr)) {
	 ...
	 * @param: clr_stat - prior status clear
	 */
	void fDev::write(uint64_t* vaddr, uint32_t len, bool poll, bool clr_stat) {
		// Check outstanding
		while (wr_cmd_cnt > (cmd_fifo_depth - cmd_fifo_thr)) {
	 ...
	 * @param: clr_stat - prior status clear
	 */
	void fDev::transfer(uint64_t* vaddr_src, uint64_t* vaddr_dst, uint32_t len_src, uint32_t len_dst, bool poll, bool clr_stat) {
		// Check outstanding
		while (rd_cmd_cnt > (cmd_fifo_depth - cmd_fifo_thr)) {
			rd_cmd_cnt = cnfg_reg[CNFG_STAT_CMD_USED_RD_REG];
			if (rd_cmd_cnt > (cmd_fifo_depth - cmd_fifo_thr))
 ...
 */
void fDev::writeContext(uint64_t r_vaddr, uint32_t r_key, uint32_t l_psn, uint32_t r_psn, uint32_t l_qpn, uint32_t l_region) {
	uint64_t offs[3]; 
	offs[0] = (((uint64_t)r_psn & 0xffffff) << 31) | (((uint64_t)l_qpn & 0xffffff) << 7) | (((uint64_t)l_region & 0xf) << 3);
	offs[1] = (((uint64_t)r_key & 0xffffff) << 24) | ((uint64_t)l_psn & 0xffffff); 
	offs[2] = (uint64_t)r_vaddr;
 ...
 * @param: pair - target queue pair
 */
void fDev::writeConnection(uint32_t r_qpn, uint32_t l_qpn, uint32_t port) {
	uint64_t offs[3];
	offs[0] = (((uint64_t)port & 0xffff) << 40) | (((uint64_t)r_qpn & 0xffffff) << 16) | ((l_qpn) & 0xffff);
...
/**
 * RDMA debug
 * TODO: Pretty expensive in hw, if needed enable it there first.
 */
void fDev::printDebugRDMA()
{
	uint64_t offs[N_RDMA_STAT_REGS]; 
	ioctl(fd, IOCTL_RDMA_STAT, &offs);
	std::cout << "-- RDMA STATISTICS ----------------------------" << std::endl;
	std::cout << std::setw(35) << "Roce crc pkg drop: \t" <<  offs[0] << std::endl;
	std::cout << std::setw(35) << "Roce psn pkg drop: \t" <<  offs[1] << std::endl;
	std::cout << std::setw(35) << "Rx word counter: \t" <<  offs[2] << std::endl;
	std::cout << std::setw(35) << "Rx pkg counter: \t" <<  offs[3] << std::endl;
	std::cout << std::setw(35) << "Tx word counter: \t" <<  offs[4] << std::endl;
	std::cout << std::setw(35) << "Tx pkg counter: \t" <<  offs[5] << std::endl;
	std::cout << std::setw(35) << "Arp rx pkg counter: \t" <<  offs[6] << std::endl;
	std::cout << std::setw(35) << "Arp tx pkg counter: \t" <<  offs[7] << std::endl;
	std::cout << std::setw(35) << "Arp request pkg counter: \t" <<  offs[8] << std::endl;
	std::cout << std::setw(35) << "Arp reply pkg counter: \t" <<  offs[9] << std::endl;
	std::cout << std::setw(35) << "Icmp rx pkg counter: \t" <<  offs[10] << std::endl;
	std::cout << std::setw(35) << "Icmp tx pkg counter: \t" <<  offs[11] << std::endl;
	std::cout << std::setw(35) << "Tcp rx pkg counter: \t" <<  offs[12] << std::endl;
	std::cout << std::setw(35) << "Tcp tx pkg counter: \t" <<  offs[13] << std::endl;
	std::cout << std::setw(35) << "Roce rx pkg counter: \t" <<  offs[14] << std::endl;
	std::cout << std::setw(35) << "Roce tx pkg counter: \t" <<  offs[15] << std::endl;
	std::cout << std::setw(35) << "Roce data rx word counter: \t" <<  offs[16] << std::endl;
	std::cout << std::setw(35) << "Roce data rx pkg counter: \t" <<  offs[17] << std::endl;
	std::cout << std::setw(35) << "Roce data tx role word counter: \t" <<  offs[18] << std::endl;
	std::cout << std::setw(35) << "Roce data tx role pkg counter: \t" <<  offs[19] << std::endl;
	std::cout << std::setw(35) << "Roce data tx host word counter: \t" <<  offs[20] << std::endl;
	std::cout << std::setw(35) << "Roce data tx host pkg counter: \t" <<  offs[21] << std::endl;
	std::cout << std::setw(35) << "Axis stream down: \t" <<  offs[22] << std::endl;
	std::cout << std::setw(35) << "Axis stream down: \t" <<  offs[23] << std::endl;
	std::cout << "-----------------------------------------------" << std::endl;
}
	// -------------------------------------------------------------------------------
	// -- Network
	 ...
	 * @param: qpn - qp number
	 */
	void fDev::writeQpn(uint32_t qpn) {
		cnfg_reg[CNFG_RDMA_QPN_REG] = _mm256_set_epi64x(0, 0, 0, qpn);
		this->qpn = qpn;
		qpn_attached = true;
	}
	 ...
	 */
	bool fDev::postWrite(uint64_t *l_addr, uint64_t *r_addr, uint32_t size) {
		if (qpn_attached) {
			uint64_t offs_0 = (uint64_t)opCode::WRITE & 0x1f;
	 ...
	 * @param: size - transfer size
	 */
	bool fDev::postRead(uint64_t *l_addr, uint64_t *r_addr, uint32_t size) {
		if (qpn_attached) {
			uint64_t offs_0 = (uint64_t)opCode::READ & 0x1f;
			uint64_t offs_1 = (((uint64_t)r_addr & 0xffff) << 48) | ((uint64_t)l_addr & 0xffffffffffff);
			uint64_t offs_2 = ((uint64_t)size << 32) | (((uint64_t)r_addr >> 16) & 0xffffffff);
			uint64_t offs_3 = 0;
	 ...
	 */
	bool fDev::postRpc(uint64_t *l_addr, uint64_t *r_addr, uint32_t size, uint64_t params) {
		if (qpn_attached) {
			uint64_t offs_0 = (uint64_t)opCode::RPC & 0x1f;
			uint64_t offs_1 = (((uint64_t)r_addr & 0xffff) << 48) | ((uint64_t)l_addr & 0xffffffffffff);
			uint64_t offs_2 = ((uint64_t)size << 32) | (((uint64_t)r_addr >> 16) & 0xffffffff);
	 ...
	 */
	void fDev::writeQpn(uint32_t qpn) {
		cnfg_reg[CNFG_RDMA_QPN_REG] = qpn;
		this->qpn = qpn;
		qpn_attached = true;
	}
