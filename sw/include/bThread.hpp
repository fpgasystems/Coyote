...
protected:
  ...
  /* Thread */
  thread c_thread; // Instance of the thread and the run-variable to check if
                   // this thread is running or not
  bool run = {false};

  /* Remote */
  std::unique_ptr<ibvQp>
      qpair; // Qpair for RDMA-operations based on this Thread
  ...
  /* eventfd */
  // Description of an Event with File Descriptor, terminator and its own thread
  // (not sure what for)
  int32_t efd = {-1};
  ...
  /* Post to controller */
  void postCmd(uint64_t offs_3, uint64_t offs_2, uint64_t offs_1,
               uint64_t offs_0);
...
public:
  ...
  // Constructor-Call
  bThread(int32_t vfid, pid_t hpid, uint32_t dev, cSched *csched = nullptr,
          void (*uisr)(int) = nullptr);
   ...
   */
  void invoke(CoyoteOper coper, sgEntry *sg_list,
              sgFlags sg_flags = {true, false, false}, uint32_t n_sg = 1);
  ...
  void *initRDMA(uint32_t buffer_size, uint16_t port,
