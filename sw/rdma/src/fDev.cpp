...
void fDev::reconfigure(uint64_t *vaddr, uint64_t len) {
	 ...
	 */
	void fDev::_read(uint64_t* vaddr, uint32_t len, bool stream, bool clr_stat, bool poll) {
		// Check outstanding
	 ...
	 * @param: clr_stat - prior status clear
	 */
	void fDev::_write(uint64_t* vaddr, uint32_t len, bool stream, bool clr_stat, bool poll) {
		// Check outstanding
	 ...
	 * @param: clr_stat - prior status clear
	 */
	void fDev::_transfer(uint64_t* vaddr_src, uint64_t* vaddr_dst, uint32_t len_src, uint32_t len_dst, bool stream, bool clr_stat, bool poll) {
...
void fDev::writeConnection(fQPair *pair, uint32_t port) {
	 ...
	 * @param: offs_3, _2, _1 - parameters
	 */
	bool fDev::postFarviewBase(fQPair *pair, uint64_t params_0, uint64_t params_1, uint64_t params_2) {
		uint64_t offs_0 = (((uint64_t)pair->local.qpn & 0xffffff) << 5) | ((uint64_t)opCode::FV & 0x1f);
		uint64_t offs_1 = params_0;
		uint64_t offs_2 = params_1;
		uint64_t offs_3 = params_2;
