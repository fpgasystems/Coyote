...
class fDev {
	...
	/* Data movement */
	void _read(uint64_t *vaddr, uint32_t len, bool stream = true, bool clr_stat = true, bool poll = true);
	void _write(uint64_t *vaddr, uint32_t len, bool stream = true, bool clr_stat = true, bool poll = true);
	void _transfer(uint64_t *vaddr_src, uint64_t* vaddr_dst, uint32_t len_src, uint32_t len_dst, bool stream = true, bool clr_stat = true, bool poll = true);
...
public:
	...
	template <typename _Vaddr = uint64_t>
	void read(_Vaddr *vaddr, uint32_t len, bool stream = true, bool clr_stat = true, bool poll = false) {
		_read((uint64_t*)vaddr, len, stream, clr_stat, poll);
	}

	template <typename _Vaddr = uint64_t>
	void write(_Vaddr *vaddr, uint32_t len, bool stream = true, bool clr_stat = true, bool poll = false) {
		_write((uint64_t*)vaddr, len, stream, clr_stat, poll);
	}

	template <typename _Vaddr = uint64_t>
	void transfer(_Vaddr *vaddr_src, _Vaddr *vaddr_dst, uint32_t len_src, uint32_t len_dst, bool stream = true, bool clr_stat = true, bool poll = false) {
		_transfer((uint64_t*)vaddr_src, (uint64_t*)vaddr_dst, len_src, len_dst, stream, clr_stat, poll);
	}
	...
	bool postFarviewBase(fQPair *pair, uint64_t params_0, uint64_t params_1, uint64_t params_2);
#endif
};
